...
class CreatePostScreen extends StatefulWidget {
  final Post? post; // If provided, we're editing; otherwise, creating
...
class _UploadPostDialogState extends State<CreatePostScreen> {
  ...
  File? _selectedImage;
  String? _existingImageBase64; // For editing mode

  @override
  void initState() {
    super.initState();
    // If editing, load existing image
    if (widget.post != null && widget.post!.imageUrl.isNotEmpty) {
      _existingImageBase64 = widget.post!.imageUrl;
    }
  }
    ...
    if (pickedFile != null) {
      setState(() {
        ...
        _existingImageBase64 =
            null; // Clear existing image when new one is picked
      });
    }
  }
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState?.saveAndValidate() ?? false) {
      ...
      final postData = {
        "title": widget.post?.title ?? "Probajemo",
      ...
      try {
        ...
        if (widget.post != null) {
          ...
          await postProvider.update(widget.post!.id, postData);
          if (!mounted) return;
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: Text(
                          widget.post != null ? 'UPDATE' : 'PUBLISH',
                          style: const TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
                            color: Colors.white,
